...
class Git {
  ...
  async push(options) {
    ...
    try {
      ...
      if (errors.isError(ex) && "data" in ex) {
        let serializedData;
        try {
  ...
  async retryOnTransientError(operation) {
    const maxRetries = 5;
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        const errorMsg = errors.isError(ex) ? ex.message : String(ex);
        let errorData = "";
        if (errors.isError(ex) && "data" in ex) {
          try {
        ...
        const isNonTransient = Git.nonTransientPatterns.some(
          (p) => p.test(fullError)
        );
        ...
        this.config.logger?.warn(
          `${operation.name} failed (attempt ${attempt + 1}/${maxRetries + 1}), retrying in ${delayMs + jitterMs}ms: ${errorMsg}`
        );

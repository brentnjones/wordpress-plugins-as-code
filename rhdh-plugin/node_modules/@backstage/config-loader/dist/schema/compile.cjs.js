...
function compileConfigSchemas(schemas, options) {
  ...
  const merged = mergeConfigSchemas(
    schemas.map((_) => {
      const copy = cloneDeep__default.default(_.value);
      traverse__default.default(copy, (s) => {
        if (typeof s.additionalProperties === "object" && s.additionalProperties !== null && Object.keys(s.additionalProperties).length === 0) {
  ...
  traverse__default.default(
    ...
    (schema, jsonPtr, _1, _2, _3, parentSchema) => {
      ...
      if (options?.noUndeclaredProperties) {
        const hasPropertyWithKey = (key) => typeof schema[key] === "object" && schema[key] !== null && Object.keys(schema[key]).length > 0;
        if (schema?.type === "object" && (hasPropertyWithKey("properties") || hasPropertyWithKey("patternProperties"))) {
